...
if(isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])){
    ...
    try{
        ...
        $conn->commit();
        $message = "<div class='alert success'>User ID $uid has been permanently removed from the system.</div>";
        
    }catch(Exception $e){
        $conn->rollback();
        $message = "<div class='alert error'>Error deleting user.</div>";
        echo "<script>alert('Error: " .addslashes($e->getMessage()) ."');</script>";
    }
}

$query = "SELECT c.customer_id, c.customer_full_name, c.customer_email, a.account_number, a.balance 
...
<div class="user-management-header">
    ...
    <p>View, manage, and modify active  accounts.</p>
</div>
...
<table class="admin-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="color: #27ae60; font-weight:bold;">₹<?php echo number_format($row['balance'], 2); ?></td>
            <td>
            <a href="admin_dashboard.php?page=users&action=delete&id=<?php echo $row['customer_id']; ?>" 
                       onclick="return confirm('WARNING: This will delete the user from db. Do you want to proceed further?')" 
                       style="color: #e74c3c; font-size: 0.9rem;">
                       [DELETE USER]
                    </a> 
...
<style>
    ...
    .admin-table th{ background: #f8f9fa; color: #7f8c8d; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 1px; padding: 15px; text-align: left; border-bottom: 2px solid #eee; }
